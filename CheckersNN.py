...
CHECKPOINT_FILE = "CheckersData/checkers_checkpoint6x6.pth"
...
if __name__ == "__main__":
    ...
    torch.save(net.state_dict(), "CheckersData/checkersModel6x6.pth")
    print("\nModel saved to checkersModel6x6.pth")

    plt.figure()
    epoch_axis = list(range(len(trian_loss_history)))
    plt.plot(epoch_axis, trian_loss_history, label="Train Loss")

    eval_axis = [i * 50 for i in range(len(test_loss_history))]
    plt.plot(eval_axis, test_loss_history, 'o-', label="Test Loss")
